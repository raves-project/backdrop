    ...
    pub(super) async fn apply_matroska(
        ...
        if format.media_kind() == MediaKind::Video {
            let mkv = matroska::open(path)
                .map_err(|e| RavesError::MatroskaError(path.display().to_string(), e))?;
            ...
            if let Settings::Video(v) = &vt.settings {
                self.resolution =
                    Some(Resolution::new(v.pixel_width as u32, v.pixel_height as u32));
                tracing::debug!(
            ...
            if let Some(duration) = mkv.info.duration {
                self.specific = Some(SpecificMetadata::Video {
